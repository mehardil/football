...
const Bet360 = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const apiUrl = 'http://127.0.0.1:8000/bet360';
  ...
  const tableHeader = (
    <thead>
      <tr>
        ...
        <th className="border px-4 py-2 bg-orange-500 text-white text-center">time</th>
    ...
    <tbody>
      {data.map((item, index) => (
        <tr key={index} className="bg-gray-100">
          ...
          <td className="border px-4 py-2 text-center">{item["Time_bet360"]}</td>
        </tr>
      ))}
    </tbody>
  );
  ...
  return (
    <div>
      ...
      <div className="container mx-auto my-4">
        <h2 className="text-2xl font-bold mb-4 text-center">BET365 Table</h2>
